...
public class Boid : MonoBehaviour
{
    ...
    public bool IsNotBoid;
    //The IsNotBoid variable is ti check if the boid is a real boid or one of the obsticals. 
    ...
    void Awake()
    {
        if(IsNotBoid == true)
        {
            //Obistocals don't need to do anything, so this can just sit here. 
        }
        else
        {
            ...
            //same code coppied from the burb scriot adjusted for a different y value 
    ...
    void LookAhead()
    {
        if (IsNotBoid == true)
        {
            //If I didn't check for this in each void, it gets mad. ie, there's a lot of errors 
        }
        else
        {
    ...
    private void FixedUpdate()
    {
        if(IsNotBoid == true)
        {
            //This proably eats a lot of processing power, and it a bad example for more sophisticalted projects, but it works here
        }
        else
        {
            ...
            vel.y = 0f;
            pos.Set(transform.position.x, -3f, transform.position.z);
